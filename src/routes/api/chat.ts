    ...
    handlers: {
      POST: async ({ request }) => {
        ...
        const result = streamText({
          ...
          onError: ({ error }) => {
            console.error("STREAMTEXT_ERROR:", JSON.stringify(error, Object.getOwnPropertyNames(error ?? {})));
          },
        });
        ...
        return result.toUIMessageStreamResponse({
          ...
          onError: (error: unknown) => {
            // Log the real error to Cloudflare Observability, but never
            // expose internal eimport { createGoogleAiProvider } from "@/lib/ai-gateway.server";
import { createFileRoute } from "@tanstack/react-router";
import { convertToModelMessages, streamText, type UIMessage } from "ai";
    ...
    handlers: {
      POST: async ({ request }) => {
        ...
        return result.toUIMessageStreamResponse({
          ...
          onError: (error: unknown) => {
            // Log the real error to Cloudflare Observability, but never
            // expose internal error detail to site visitors.
            console.error("STREAM_RESPONSE_ERROR:", JSON.stringify(error, Object.getOwnPropertyNames(error ?? {})));
            return "Sorry — the assistant is unavailable right now. Please try again shortly.";
          },
        });
      },
    },
